    ...
    fn check(&self, node_state: &NodeState) -> bool {
        match self {
            NodeStatePredicate::EqualKeyValue(key, expected_value) => {
                let versioned_value =
                    node_state.get_versioned(key).expect("Key is expected to be present");
...
impl Simulator {
    ...
    pub async fn execute(&mut self, operations: Vec<Operation>) {
        for operation in operations {
            ...
            match operation {
                Operation::AddNode { chitchat_id, peer_seeds } => {
                    self.spawn_node(chitchat_id, peer_seeds).await;
                }
                Operation::InsertKeysValues { chitchat_id, keys_values } => {
                ...
                Operation::NodeStateAssert {
                    ...
                    let chitchat = self.node_handles.get(&server_chitchat_id).unwrap().chitchat();
                    ...
                    if let Some(timeout) = timeout_opt {
                        ...
                        tokio::time::timeout(timeout, async move {
                            loop {
                                {
                                    let chitchat_guard = chitchat_clone.lock();
    ...
    pub async fn spawn_node(
        ...
        let seed_nodes: Vec<_> = peer_seeds
            .unwrap_or_else(|| self.node_handles.keys().cloned().collect::<Vec<ChitchatId>>())
            .iter()
        ...
        let handle = spawn_chitchat(config, Vec::new(), &self.transport).await.unwrap();
...
async fn test_simple_simulation_insert() {
    ...
    let operations = vec![
        Operation::AddNode { chitchat_id: chitchat_id_1.clone(), peer_seeds: None },
        Operation::AddNode { chitchat_id: chitchat_id_2.clone(), peer_seeds: None },
...
async fn test_marked_for_deletion_gc_with_network_partition_2_nodes() {
    ...
    let operations = vec![
        ...
        Operation::DeleteKey { chitchat_id: chitchat_id_1.clone(), key: "key_a".to_string() },
...
async fn test_marked_for_deletion_gc_with_network_partition_4_nodes() {
    ...
    let peer_seeds = vec![chitchat_id_1.clone(), chitchat_id_2.clone(), chitchat_id_3.clone()];
    let operations = vec![
        Operation::AddNode {
            chitchat_id: chitchat_id_1.clone(),
            peer_seeds: Some(peer_seeds.clone()),
...
async fn test_simple_simulation_heavy_insert_delete() {
    ...
    for chitchat_id in chitchat_ids.iter() {
        ...
        for key in key_names.iter() {
            ...
            let keys_entry =
                keys_values_inserted_per_chitchat_id.entry(chitchat_id.clone()).or_default();
    ...
    for (chitchat_id, keys) in keys_values_inserted_per_chitchat_id.clone().into_iter() {
        for key in keys {
            let check_operation = Operation::DeleteKey { chitchat_id: chitchat_id.clone(), key };
            simulator.execute(vec![check_operation]).await;
        }
    }
