...
const STREAM_OP_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(10);
...
impl NetConnection for MsQuicNetConnection {
    ...
    fn local_addr(&self) -> SocketAddr {
        self.inner.get_local_addr().unwrap_or_else(|_| "0.0.0.0:0".parse().unwrap())
    }

    fn remote_addr(&self) -> SocketAddr {
        self.inner.get_remote_addr().unwrap_or_else(|_| "0.0.0.0:0".parse().unwrap())
    }
    ...
    async fn send(&self, bytes: &[u8]) -> anyhow::Result<()> {
        ...
        match tokio::time::timeout(STREAM_OP_TIMEOUT, stream.flush()).await {
            Ok(result) => result?,
            Err(_) => anyhow::bail!("Timeout flushing stream: took more {STREAM_OP_TIMEOUT:?}"),
        }
        match tokio::time::timeout(STREAM_OP_TIMEOUT, stream.close()).await {
            Ok(result) => result?,
            Err(_) => anyhow::bail!("Timeout closing stream: took more {STREAM_OP_TIMEOUT:?}"),
        }
        Ok(())
    }
